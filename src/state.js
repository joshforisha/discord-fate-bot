  ...
  return entityIndexNamed(entityStart).then((e) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (roughMatches.length > 1) {
        return reject(new Error(`I found multiple aspects starting with "${asp}"`))
      }
      if (roughMatches.length < 1) {
        return reject(new Error(`I couldn't find an aspect starting with "${asp}"`))
      }
...
export function entityIndexNamed (entityStart) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (roughMatches.length > 1) {
      return reject(new Error(`I found multiple entities starting with "${ent}"`))
    }
    if (roughMatches.length < 1) {
      return reject(new Error(`I couldn't find an entity starting with "${ent}"`))
    }
  ...
  return entityIndexNamed(entityStart).then((e) => {
    ...
    return new Promise((resolve, reject) => {
      if (!('tracks' in entities[e])) {
        return reject(new Error(`No tracks found for entity "${entities[e].name}"`))
      }

      const exactMatch = entities[e].tracks.findIndex(
        ({ type }) => type.toLowerCase() === trk
      )
      if (exactMatch > -1) return resolve([e, exactMatch])

      const roughMatches = entities[e].tracks.reduce(
        (ts, { type }, t) =>
          type.toLowerCase().startsWith(trk) ? [...ts, t] : ts,
        []
      )
      if (roughMatches.length > 1) {
        return reject(new Error(`I found multiple tracks starting with "${trk}"`))
      }
      if (roughMatches.length < 1) {
        return reject(new Error(`I couldn't find a track starting with "${trk}"`))
      }
      resolve([e, roughMatches[0]])
    })
  })
